...
def test_build_pages_site_separates_sources_and_preserves_tag_membership(tmp_path: Path) -> None:
    ...
    assert "pageSize=50" in app_js
    assert 'id="page-size"' in index_html
    assert '<option value="50" selected>50</option>' in index_html
    assert ".status-row{display:flex" in style_css
    assert "th.rating-count,td.rating-count{text-align:right;padding-right:48px}" in style_css
    assert stats == {
